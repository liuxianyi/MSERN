...
class my_data_set(Dataset):
    def __init__(self, opt, train=True):
        ...
        self.opt = opt
        self.ANP = np.load(opt.ANP.format(name), allow_pickle=True).item()
        self.Text = np.load(opt.Text.format(name), allow_pickle=True).item()
        self.name_list_file = np.load(opt.name_list, allow_pickle=True)
        ...
        for idx, name in self.name_list_file:
            video_cls[re.findall('')].append(idx)
